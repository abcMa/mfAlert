...
widget = {
    ...
    */
    ...
    _init: function () {
        ...
        var options = {
            view: Iptools.DEFAULTS.currentNewView,
        };
    ...
    _initListApplet: function (options) {
        Iptools.uidataTool._getApplet({
            ...
            if (ds.applet.type == "list") {
                ...
                $(options.panel).parent().find("table thead tr").append("<th style='width:100px;'>操作</th>");
                $(options.panel).parent().find("table tbody tr").each(function (key, obj) {
    ...
    _setListAppletContent: function (options) {
        ...
        widget._setGroupListButtonsPanel({
    ...
    _setGroupListRelatedControl: function (options) {
        ...
        Iptools.uidataTool._getApplet({
            async: false,
            applet: options.data.applet.createApplet,
        }, function (ds) {
            if (ds.applet.type == "detail") {
    ...
    _setGroupListMainData: function (options) {
        var clickColumns = [];
        var clickColumnViews = [];
        var reDetailWidget = widget._setGroupListRelatedControl(Iptools.Tool._extend(options, { valueId: "*" }));
        var mld = document.createElement("div");
        $(mld).attr("id", options.container);
        $(mld).addClass("table-responsive");
        var tbl = document.createElement("table");
    ...
    _setGroupListButtonsPanel: function (options) {
        ...
        $(a2).append(sp2, "提示");
        $(li2).append(a2);
        $(options.holder).find(".button-nav").append(li, li2);
    },
    _setGroupDetailButtonsPanel: function (options) {
        $(options.holder).find(".button-nav").html("");
        var a = document.createElement("a");
        var li = document.createElement("li");
        var sp = document.createElement("span");
        $(a).css("cursor", "pointer");
        $(a).addClass("groupSaveDataBtn");
        $(a).data("form", options.form);
        $(a).attr("data-loading-text", "<span class='icon-save icon-spin' style='margin-right:10px'></span>保存中");
        $(sp).addClass("icon-save");
        $(sp).css("margin-right", "10px");
        $(a).append(sp, "保存");
        $(li).append(a);
    ...
    _setDetailAppletContent: function (options) {
        ...
        if (options.type == "primary") {
            widget._setGroupDetailButtonsPanel({
                applet: options.data.applet.id,
                condition: options.condition,
                type: options.type,
                container: options.container,
        ...
        widget._addEventListener({
            ...
            event: function (e) {
                ...
                if (widget._UIDEFAULFS.DataGroupEditingStatus) {
                    ...
                    $("#" + me).bootstrapValidator('validate');
                    if ($("#" + me).data("bootstrapValidator") && $("#" + me).data("bootstrapValidator").isValid()) {
                        var dataImg = $("#" + me).find(".lightboxLink");
                        var datafile = $("#" + me).find(".file-data");
                        dataImg.each(function () {
                            var ctrl = $(this);
                        ...
                        if ($.isEmptyObject(widget._UIDEFAULFS.DataCurrentSets[me])
                            ...
                            Iptools.uidataTool._addGroupAppletData({
    ...
    _enableEditUnLock: function () {
        widget._addEventListener({
            ...
            event: function () {
                ...
                $(span).css("margin-right", "10px");
                me.append(span, "保存");
                me.attr("class", "groupSaveDataBtn");
                me.attr("data-loading-text", "<span class='icon-save icon-spin' style='margin-right:10px'></span>保存中");
    ...
    _clearGroupDataNew: function (options) {
        for (var i = 0; i < options.controls.length; i++) {
            $("#" + widget._UIDEFAULFS.groupListForm).data("bootstrapValidator")
                .updateStatus($(options.controls[i]).attr("name"), "NOT_VALIDATED");
