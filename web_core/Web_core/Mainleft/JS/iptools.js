        ...
        **/
        ...
        _sessionCheck: function () {
            if (Iptools.getDefaults({ key: "token" }) == null) {
                //从官网登陆-没传token
                if (!Iptools.Tool.$getUrlParameter("token")) {
                    self.location = "http://" + Iptools.DEFAULTS.crmDomain + Iptools.DEFAULTS.hostPath;
                }
                if (Iptools.Tool.$getUrlParameter("token").length !== 0) {
                    window.sessionStorage.setItem("userId", Iptools.Tool.$getUrlParameter("userId"));
                    window.sessionStorage.setItem("tenantId", Iptools.Tool.$getUrlParameter("t"));
                    window.sessionStorage.setItem("token", Iptools.Tool.$getUrlParameter("token"));
                    window.sessionStorage.setItem("loginName", Iptools.Tool.$getUrlParameter("loginName"));
                    window.sessionStorage.setItem("userTitle", Iptools.Tool.$getUrlParameter("userTitle"));
                    this._transSession();
                }
            }
        },
        _setUserInvironment: function () {
            Iptools.GetJson({
                ...
                if ($("#application_user").length) {
                    ...
                    $(".user_headpic").attr("src", (r.employee.headPic ? r.employee.headPic : "../Content/Image/defaultHead.png"));
                }
            });
            Iptools.GetJson({
                ...
                if (data) {
                    if (Iptools.Tool._checkNull(data.shortName)) {
                        document.title = data.shortName;
                        $("#application_company").html(data.shortName);
                    } else {
                        document.title = "工作台";
                        $("#application_company").html("工作台");
                    }
                }
            });
        },
    ...
    **/
    Iptools.Tool = {
        ...
        FrameHeight: function () {
            $(widget._UIDEFAULFS.tabContents).find("iframe").each(function (key, obj) {
                if ($(obj).parents(".tab-pane:first").hasClass("active")) {
                    var fhold = obj.contentWindow.document;
        ...
        _jumpView: function (options, beforeLoad, afterLoad, onLoad) {
            ...
            widget._initView(options, onLoad);
            if (afterLoad) {
                afterLoad();
            }
        },
        ...
        _setBreadCondition: function () {
        },
        _setBreadDetectValue: function () {
        },
        ...
        _CloseTab: function (options, beforeLoad, afterLoad) {
            ...
            widget._CloseTab(options, afterLoad);
        },
