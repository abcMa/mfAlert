...
(function () {
    ...
    var toolBase = {
        ...
        _enableProto: function () {
            ...
            jQuery.fn.loading = function (options) {
                ...
                $(d).addClass("load-data-container col-lg-12").css("height", options.size + "px");
                var img = document.createElement("img");
                $(img).attr("src", options.url).attr("alt", "").css("height", options.size + "px");
                me.html($(d).append(img));
            }
        }
    };
    ...
    Iptools.uidataTool = {
            ...
            Iptools.GetJson({
                ...
                if (r && r.retcode != "fail") {
                    ...
                    ds.applet = {
                        ...
                        isPublic: r.applet.isPublic,
                        innerConditionOperator: r.applet.innerConditionOperator
                    };
                    if (r.controls && r.controls.length > 0) {
                        $.each(r.controls, function (key, obj) {
                            ds.controls.push({
                                ...
                                isSeparate: obj.isSeparate,
                                separateTitle: obj.separateTitle,
                                isTrigger: obj.isTrigger,
                                triggerFormula: obj.triggerFormula,
                    ...
                    if (r.listColumns && r.listColumns.length > 0) {
                        $.each(r.listColumns, function (key, obj) {
                            ds.columns.push({
                                ...
                                field: obj.field,
                                size: obj.size,
                                isSeprate: obj.isSeprate,
                                seprateTitle: obj.seprateTitle,
                                isNewLine: obj.isNewLine,
                                virtualType: obj.type,
                                virtualName: obj.virtualColumnName
                            });
                        });
                    }
        ...
        _getUserlistAppletData: function (options) {
            ...
            Iptools.GetJson({
                ...
                if (r && r.retcode != "fail") {
                    ...
                    ds.statisticData = r.statisticData;
                    ipromise.resolve(ds);
                } else {
                    ipromise.reject();
                }
        ...
        _getAppletColumnValueList: function (options) {
            var ipromise = $.Deferred();
            Iptools.GetJson({
        ...
        _exportExcel: function (options) {
            ...
            if (Iptools.Tool._checkNull(options.virtualCondition)) {
                data["virtualCondition"] = options.virtualCondition;
            }
            Iptools.PostJson({
                async: options.async == false ? false : true,
                url: "service/exportExcelDataGetList",
                data: data
            }).done(function (r) {
                if (r && r.retcode == "ok") {
                    ipromise.resolve(Iptools.DEFAULTS.serviceUrl + r.filePath);
                } else {
                    ipromise.reject();
                }
            }).fail(function () {
        ...
        _buildReport: function (options) {
            var emptyHtml = '<div class="noDataArea emptyReport"><img src="../Content/Image/emptyReport.png" class="emptyImg" style="width:100px;"><span>暂无统计数据</span></div>';
            var ipromise = $.Deferred();
            Iptools.uidataTool._getApplet({
                ...
                if (ds && ds.applet.type == "report") {
                    ...
                    if (Iptools.Tool._checkNull(options.endDate)) {
                        para["reportDateEnd"] = options.endDate;
                    }
                    ...
                    Iptools.GetJson({
                        ...
                        if (r && r.retcode != "fail") {
                            ...
                            if (!$.isEmptyObject(report)) {
                                ...
                                if (report.report.xAxisType == "group" || report.report.xAxisType == "cusGroup") {
                                    switch (report.report.type) {
                                        ...
                                        case "bar":
                                            ...
                                            if (options.showReport) {
                                                if (reportBody.series.length === 0 || reportBody.series[0].data.length === 0) {
                                                    $("#" + options.container).css("position", "relative").html($(emptyHtml).css({
                                                        "position": "absolute",
                                                        "top": "50%",
                                                        "left": "50%",
        ...
        _pushMessage: function (options) {
            var param = {
                token: Iptools.DEFAULTS.token,
                channel: Iptools.DEFAULTS.tenantId + "_" + options.channel,
                data: JSON.stringify(options)
            }
            Iptools.PostJson({
                url: "basic/sockets/push",
                data: param
            });
        }
    };
    ...
    Iptools.Tool = {
        ...
        _pushListen: function (channel, func) {
            parent.Iptools.Tool._pushListen(channel, func);
        }
    };
})();