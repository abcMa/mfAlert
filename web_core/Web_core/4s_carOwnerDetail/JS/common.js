...
//        owner:window.sessionStorage.getItem('userTitle')
    ...
    _bindingDomEvent: function () {//绑定事件
        common._cancelSaveNote();//点击取消，按钮消失
        common._saveTask();//保存新建的任务
        common._saveNote();//保存新建的笔记
        common._focusTipsText();//聚焦在tips中的textarea
...
//                var taskRemind = common._getBeforeDay(taskDue,remindDay);
                    var data = '{"'+common._UIDEFAULFS.taskDetailRoot + ':title":"'+taskTitle+'","'+
                        common._UIDEFAULFS.taskDetailRoot+':customer_id":"'+widget._UIDEFAULFS.cusId+'",';
    ...
    _saveNote: function(){
        common._addEventListener({
            ...
            event: function (e) {
                ...
                var data = '{"'+common._UIDEFAULFS.noteDetailRoot + ':content":"'+text+'","'+common._UIDEFAULFS.noteDetailRoot+':customer_id":"'+widget._UIDEFAULFS.cusId+'"}';
                Iptools.uidataTool._addAppletData({
    ...
    _clickSendMsg:function(){
        common._addEventListener({
            ...
            event: function (e) {
                ...
                me.button("loading");
                if($('.selectSend').data("phone")){
                    Iptools.PostJson({
                        url:"basic/sendSms",
                        data:{
                            token:Iptools.DEFAULTS.token,
                            phoneList:$('.selectSend').data("phone"),
        ...
        common._addEventListener({
            ...
            event: function (e) {
                ...
                var data = '{"'+trace._UIDEFAULFS.contactTraceDeRoot+':title":"'+title+'","'+
                    ...
                    trace._UIDEFAULFS.contactTraceDeRoot+':customer_id":"'+widget._UIDEFAULFS.cusId+'","'+
