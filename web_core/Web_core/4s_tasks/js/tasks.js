                            ...
                            for (var i = 0; i < r.data.records.length; i++) {
                                ...
                                if (rec[r.rootLink + ":status"] && rec[r.rootLink + ":status"].id) {
                                    if (rec[r.rootLink + ":status"].id == "2") {
                                        rec[r.rootLink + ":title"] = "<span class='taskCheckStyle' data-task='true' " +
                                            "data-id='" + rec[r.rootLink + ":id"] + "'><input type='checkbox' class='taskCheck blueCheckbox'/></span>"
                                            + "<span class='taskNameTitle' data-id='" + rec[r.rootLink + ":id"] + "'>" + rec["task:title"] + "</span>";
                                    } else {
                                        rec[r.rootLink + ":title"] = "<span class='taskCheckStyle' data-task='false' data-id='"
                                            + rec[r.rootLink + ":id"] + "'><input type='checkbox' class='taskCheck blueCheckbox'/></span>"
                                            + "<span class='taskNameTitle' data-id='" + rec[r.rootLink + ":id"] + "'>" + rec["task:title"] + "</span>";
                                    }
                                } else {
                                    rec[r.rootLink + ":title"] = "<span class='taskCheckStyle' data-task='false' data-id='"
                                ...
                                if (rec[r.rootLink + ":car_id"] && rec[r.rootLink + ":car_id"]["id"]) {
                                    ...
                                    rec[r.rootLink + ":car_id"] = "<span class='taskCarNameTitle' data-id='" + linkId + "' data-car-id=" + rec[r.rootLink + ":car_id"]["id"] + ">" + rec[r.rootLink + ":car_id"]["name"] + "</span>";
                                }
                            }
                        } else {
    ...
    _setPanel: function () {
        ...
        Iptools.uidataTool._getCustomizeApplet({
            ...
            Iptools.uidataTool._getUserlistAppletData({
                ...
                $("#addTaskDeals").html("");
                var columnsHtml = "", statusHtml = "", statusWHtml = "", statusNotHtml = "", task = r.rootLink, Taskid = "", taskTitle = "", taskOwner = "", taskTime = "", num = 0, numT = 0, numW = 0;
            ...
            event: function () {
                ...
                if ($("#editNewTaskForm").data("bootstrapValidator").isValid()) {
                    ...
                    if (taskWidget._UIDEFAULFS.addNewTaskCustomerId != "") {
                        taskStr[taskDatas + ":contact_id"] = taskWidget._UIDEFAULFS.addNewTaskCustomerId;
                    }
                    if (taskWidget._UIDEFAULFS.addTaskCarOwnerId != "") {
                        taskStr[taskDatas + ":customer_id"] = taskWidget._UIDEFAULFS.addTaskCarOwnerId;
                    }
            ...
            event: function () {
                ...
                if ($("#editTaskForm").data("bootstrapValidator").isValid()) {
                    ...
                    if ($("#modalTaskOwner option:selected").val() != "") {
                        param[taskDatas + ":owner"] = taskWidget._UIDEFAULFS.addownerId;
                    }
                    if (taskWidget._UIDEFAULFS.addTaskCustomerId != "") {
                        param[taskDatas + ":contact_id"] = taskWidget._UIDEFAULFS.addTaskCustomerId;
                    }
                    if (taskWidget._UIDEFAULFS.addTaskCarOwnerId != "") {
                        param[taskDatas + ":customer_id"] = taskWidget._UIDEFAULFS.addTaskCarOwnerId;
                    }
    ...
    _editTaskFunction: function () {
        ...
        Iptools.GetJson({
            ...
            if (r.detailData[task + ':customer_id']) {
                carOwner_ids = r.detailData[task + ':customer_id'].id;
                carOwner_idHtml = r.detailData[task + ':customer_id'].name;
            }
            if (r.detailData[task + ':deal_id']) {
                deal_ids = r.detailData[task + ':deal_id'].id;
                deal_idHtml = r.detailData[task + ':deal_id'].name;
            }
